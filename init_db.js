...
const hostsToTry = [process.env.DB_HOST, 'localhost', '127.0.0.1', 'YADU1C', 'localhost\\SQLEXPRESS', 'YADU1C\\SQLEXPRESS'];
const uniqueHosts = [...new Set(hostsToTry.filter(Boolean))];

async function testConnections() {
    for (const host of uniqueHosts) {
        console.log(`Trying connection to server: "${host}"...`);
        const config = {
            user: process.env.DB_USER || 'sa',
            password: process.env.DB_PASSWORD || 'sonu534y',
            server: host,
            port: host.includes('\\') ? undefined : (parseInt(process.env.DB_PORT, 10) || 1433),
            database: 'master',
        ...
        try {
            ...
            console.log(` SUCCESS! Connected to SQL Server at "${host}"`);
            ...
            await pool.request().query(`
                ...
                END
            `);
            console.log('Database school_next_pro created/verified.');
            await pool.close();

            // Connect to school_next_pro
            const appConfig = { ...config, database: 'school_next_pro' };
            const appPool = await mssql.connect(appConfig);
    ...
    console.error('❌ Could not connect to any SQL server host. Check if SQL Server service is running.');
    process.exit(1);
}
